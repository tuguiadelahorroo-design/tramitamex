---
titulo: Opinión de cumplimiento
tituloLargo: 'Opinión de cumplimiento del SAT (32-D): cómo obtenerla gratis'
tituloSeo: 'Opinión de cumplimiento SAT (32-D) gratis'
descripcion: 'Genera tu opinión de cumplimiento de obligaciones fiscales del SAT gratis en línea con contraseña o e.firma. Paso a paso, vigencia y qué hacer si sale negativa.'
resumen: 'La opinión de cumplimiento (artículo 32-D) indica si estás al corriente con el SAT. Te explicamos cómo generarla gratis en línea con tu contraseña o e.firma, su vigencia y qué hacer si sale en sentido negativo.'
categoria: 'SAT e impuestos'
institucion: 'sat'
nivel: 'Federal'
costo: 'Gratis'
modalidad: 'En línea'
tiempo: 'Inmediato'
vigencia: '30 días (3 meses para estímulos)'
urlOficial: 'https://www.sat.gob.mx/consultas/20777/consulta-tu-opinion-de-cumplimiento-de-obligaciones-fiscales'
urlOficialTexto: 'Opinión de cumplimiento — Portal del SAT'
actualizado: 2026-06-17
revisadoPor: 'el equipo editorial de TramitaMex'
destacado: true
orden: 14
relacionados:
  - constancia-de-situacion-fiscal
  - firma-electronica
  - rfc
pasos:
  - titulo: 'Abre la aplicación de la opinión'
    detalle: 'Ingresa al Portal del SAT o directamente a la aplicación de la opinión de cumplimiento.'
  - titulo: 'Da clic en Iniciar'
    detalle: 'Comienza el trámite desde la aplicación.'
  - titulo: 'Autentícate'
    detalle: 'Entra con tu contraseña del SAT (RFC + contraseña) o con tu e.firma. Cualquiera de las dos funciona.'
  - titulo: 'Captura el captcha'
    detalle: 'Escribe el texto de la imagen que muestra la pantalla.'
  - titulo: 'Envía la solicitud'
    detalle: 'Da clic en Enviar para que el sistema consulte tu situación fiscal.'
  - titulo: 'Guarda o imprime tu opinión'
    detalle: 'El sistema genera la opinión de inmediato en PDF; descárgala o imprímela.'
faq:
  - pregunta: '¿Cuánto cuesta la opinión de cumplimiento?'
    respuesta: 'Es totalmente gratuita. El SAT no cobra por emitirla.'
  - pregunta: '¿Necesito e.firma o me sirve la contraseña?'
    respuesta: 'Cualquiera de las dos funciona. Puedes generarla solo con tu contraseña del SAT (RFC + contraseña) o con tu e.firma.'
  - pregunta: '¿Tengo que ir a una oficina o sacar cita?'
    respuesta: 'No. El trámite es 100% en línea, sin cita, desde el Portal del SAT.'
  - pregunta: '¿Cuánto tarda en emitirse?'
    respuesta: 'Es inmediata. Se genera y se descarga o imprime en el momento.'
  - pregunta: '¿Cuánto tiempo es válida?'
    respuesta: 'Tres meses para estímulos o subsidios y 30 días naturales en los demás casos, contados desde su emisión.'
  - pregunta: 'Mi opinión salió negativa pero estoy al corriente, ¿qué hago?'
    respuesta: 'Presenta una aclaración en el Portal del SAT (Mi portal / Servicios por internet / Aclaraciones / Solicitud). Es gratuita y la puedes ingresar en cualquier momento.'
  - pregunta: '¿Para qué me la pueden pedir?'
    respuesta: 'Principalmente para contratar con el gobierno (adquisiciones, servicios, obra pública), recibir subsidios o estímulos, y para créditos o relaciones comerciales.'
  - pregunta: '¿Puede un tercero consultar mi opinión?'
    respuesta: 'Sí, si autorizas que el resultado sea público, un tercero puede consultarla mediante la consulta pública del SAT.'
fuentes:
  - titulo: 'Obtén tu Opinión del cumplimiento — SAT'
    url: 'https://www.sat.gob.mx/consultas/20777/consulta-tu-opinion-de-cumplimiento-de-obligaciones-fiscales'
  - titulo: 'Aclara las inconsistencias de tu Opinión del cumplimiento — SAT'
    url: 'https://www.sat.gob.mx/tramites/20778/aclara-la-informacion-de-tu-opinion-de-cumplimiento-de-obligaciones-fiscales'
  - titulo: 'Ficha del trámite (SAT2252) — gob.mx'
    url: 'https://www.gob.mx/tramites/ficha/opinion-del-cumplimiento-de-obligaciones-fiscales/SAT2252'
  - titulo: 'Valida la autenticidad de tu opinión — SAT'
    url: 'https://www.sat.gob.mx/consulta/20957/valida-la-autenticidad-de-la-informacion-de-tu-opinion-del-cumplimiento-de-obligaciones-fiscales'
---

## Qué es la opinión de cumplimiento

La **Opinión del cumplimiento de obligaciones fiscales** es un documento que emite el SAT y que indica de manera instantánea si estás al corriente con tus obligaciones fiscales. Su fundamento es el **artículo 32-D del Código Fiscal de la Federación**. Puede salir en dos sentidos:

- **Positiva:** estás al corriente; cumples todos los requisitos que revisa la autoridad.
- **Negativa:** tienes adeudos, omisiones u otras inconsistencias pendientes de regularizar.

Para que salga **positiva**, la autoridad revisa que estés inscrito y activo en el [RFC](/rfc/), al corriente en tus declaraciones, que no aparezcas en el listado del artículo 69-B (operaciones simuladas) y que no tengas créditos fiscales firmes o exigibles.

## Para qué sirve

- **Contratar con el gobierno:** es requisito para celebrar contratos con cualquier autoridad o ente público.
- **Adquisiciones, arrendamientos, servicios u obra pública** con el sector público.
- **Solicitar subsidios o estímulos** fiscales o económicos.
- **Trámites de crédito** y financiamiento (bancos y entidades la solicitan).
- **Relaciones comerciales** con proveedores y clientes que la exigen por compliance.

## Requisitos

- **RFC** vigente.
- **Contraseña** del SAT **o** **[e.firma](/firma-electronica/)** vigente. Cualquiera de las dos es suficiente.

## Documentos necesarios

No se entregan documentos físicos. Solo necesitas tus credenciales:

- **Contraseña** del SAT, **o**
- **e.firma** (archivos .cer y .key más la contraseña de la clave privada).

El resultado se obtiene como un **PDF** que puedes guardar o imprimir.

## Costo

Es **completamente gratuito**. El SAT no cobra por generar la opinión.

## Dónde se hace

**En línea**, en el Portal del SAT. No es necesario acudir a una oficina. La aplicación genera la opinión al momento, autenticándote con contraseña o e.firma.

## Cómo sacar cita

**No se requiere cita.** El trámite es 100% en línea, sin necesidad de agendar ni acudir a una oficina del SAT.

## Tiempo de respuesta

**Inmediato.** La opinión se genera y se puede descargar o imprimir en el momento, en línea.

## Vigencia

La opinión refleja tu situación fiscal **a la fecha de emisión**. La vigencia de una opinión positiva depende del uso que le des:

| Para qué la usas | Vigencia |
|---|---|
| Estímulos o subsidios | 3 meses |
| Contratación, proveedores, créditos (demás casos) | 30 días naturales |

> [!AVISO]
> En la práctica casi siempre te la pedirán **reciente**. Genérala con no más de 30 días de antigüedad para evitar que te la rechacen.

## Problemas comunes

- **Negativa por adeudos:** existen créditos fiscales firmes o exigibles. Paga, garantiza el adeudo o entra a un convenio de pago en parcialidades.
- **Negativa por omisión de declaraciones:** presenta todas las declaraciones pendientes y vuelve a generarla.
- **Estado fiscal no activo:** RFC suspendido o cancelado; reactiva o actualiza tu situación en el RFC.
- **Aparición en el 69-B:** estar en el listado definitivo genera opinión negativa.
- **Problemas de acceso:** contraseña bloqueada o e.firma vencida; primero renueva o desbloquea tus credenciales.

> [!CONSEJO]
> Si tu opinión sale **negativa pero crees estar al corriente**, presenta una **aclaración** en el Portal del SAT (Mi portal / Servicios por internet / Aclaraciones / Solicitud). Es gratuita y puedes ingresarla en cualquier momento.
