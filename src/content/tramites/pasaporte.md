---
titulo: Pasaporte
tituloLargo: 'Pasaporte mexicano: costos 2026, requisitos y cómo sacar cita'
descripcion: 'Saca o renueva tu pasaporte mexicano en la SRE: costos 2026 por vigencia, requisitos, cómo agendar cita gratis y paso a paso. Trámite personal.'
resumen: 'El pasaporte lo expide la SRE solo con cita previa (gratuita) y de forma personal. Te explicamos los costos 2026 por vigencia, los requisitos para adultos y menores, y cómo agendar tu cita sin intermediarios.'
categoria: 'Identidad y documentos'
institucion: 'sre'
nivel: 'Federal'
costo: 'Desde $875 (según vigencia)'
modalidad: 'Presencial con cita'
tiempo: 'Mismo día'
vigencia: '3, 6 o 10 años'
urlOficial: 'https://www.gob.mx/pasaporte/'
urlOficialTexto: 'Portal del Pasaporte — gob.mx'
actualizado: 2026-06-17
destacado: true
orden: 6
faq:
  - pregunta: '¿La cita para el pasaporte tiene costo?'
    respuesta: 'No. Agendar la cita es totalmente gratuito; solo se paga el derecho del pasaporte según la vigencia. Quien cobre por la cita es un intermediario no autorizado.'
  - pregunta: '¿Puedo enviar a un gestor a hacer el trámite por mí?'
    respuesta: 'No. El trámite es estrictamente personal; el solicitante debe presentarse para la toma de foto, firma y huellas.'
  - pregunta: '¿Entregan el pasaporte el mismo día?'
    respuesta: 'En la mayoría de los casos sí, el mismo día de la cita en oficinas dentro de México. En consulados el tiempo puede variar.'
  - pregunta: '¿Quiénes tienen descuento del 50%?'
    respuesta: 'Las personas de 60 años o más, las personas con discapacidad comprobable y los trabajadores agrícolas temporales del programa con Canadá.'
  - pregunta: 'Para renovar, ¿necesito todos los documentos de nuevo?'
    respuesta: 'Normalmente no: basta el pasaporte anterior y el comprobante de pago, salvo que cambien tus datos o lo hayas perdido.'
  - pregunta: '¿Qué necesita un menor de edad?'
    respuesta: 'Acta de nacimiento certificada, presencia del menor y de ambos padres o tutores con identificación, y la firma en persona del formato OP-7.'
  - pregunta: 'Vivo en el extranjero, ¿cómo lo tramito?'
    respuesta: 'En el consulado o embajada de México que te corresponda, agendando cita a través del sistema MiConsulado en citas.sre.gob.mx.'
  - pregunta: '¿Qué vigencia me conviene?'
    respuesta: 'El de 10 años es el más económico por año, pero solo para mayores de edad; el de 3 o 6 años es útil si prevés cambios o para menores.'
fuentes:
  - titulo: 'Pasaporte Mexicano — SRE (gob.mx/sre)'
    url: 'https://www.gob.mx/sre/acciones-y-programas/pasaporte-mexicano'
  - titulo: 'Portal del Pasaporte Mexicano (gob.mx/pasaporte)'
    url: 'https://www.gob.mx/pasaporte/'
  - titulo: 'Sistema oficial de citas SRE'
    url: 'https://citas.sre.gob.mx/'
  - titulo: 'Directorio de Delegaciones de la SRE'
    url: 'https://portales.sre.gob.mx/directorio/delegaciones'
---

## Qué es

El **pasaporte mexicano** es el documento oficial de viaje e identidad que expide la Secretaría de Relaciones Exteriores (SRE). Acredita la nacionalidad y la identidad de la persona titular ante las autoridades de México y de otros países. Es personal e intransferible. El más común es el **pasaporte ordinario**; existen también el oficial y el diplomático, solo para personas con cargos específicos.

## Para qué sirve

- Viajar al extranjero y acreditar tu identidad y nacionalidad mexicana en otros países.
- Solicitar visas en embajadas y consulados.
- Funcionar como identificación oficial dentro y fuera de México.
- Realizar trámites migratorios de entrada y salida del país.

## Requisitos

- Realizar el trámite de manera **personal** (la SRE no acepta gestores ni intermediarios).
- Tener una **cita previa** (obligatoria y gratuita).
- Acreditar la **nacionalidad mexicana** y la **identidad** con identificación oficial vigente.
- Presentar el **comprobante de pago** de derechos.

Para **menores de edad**: presencia del menor y de **ambos padres o tutores** con identificación, y firma en persona del formato de consentimiento **OP-7**.

## Documentos necesarios

**Adultos (primera vez):**

- **Acta de nacimiento** certificada (o carta de naturalización para personas naturalizadas).
- **Identificación oficial vigente con fotografía** (INE, cédula profesional, cartilla militar, etc.).
- **CURP** (se recomienda llevarla).
- **Comprobante de pago**. La fotografía se toma en la propia oficina.

**Renovación:** pasaporte anterior (original) y comprobante de pago.

## Costo

El pasaporte **sí tiene costo**, fijado por la Ley Federal de Derechos. Montos aproximados vigentes en 2026:

| Vigencia | Costo 2026 | Con 50% de descuento |
|---|---|---|
| 3 años | $1,750 | $875 |
| 6 años | $2,380 | $1,190 |
| 10 años (solo mayores de edad) | $4,120 | $2,060 |
| 1 año (solo menores de 3 años) | $920 | $460 |

El descuento del 50% aplica a **personas de 60 años o más**, **personas con discapacidad** comprobable y **trabajadores agrícolas temporales** del programa con Canadá. Algunas oficinas ofrecen trámite urgente con un recargo aproximado del 30%.

## Dónde se hace

- **En México:** en las **Delegaciones de la SRE** y **Oficinas de Enlace** estatales y municipales, solo con **cita previa**.
- **En el extranjero:** en las **embajadas y consulados** de México, mediante el sistema **MiConsulado**.

> Evita gestores o "servicios exprés" externos: son fraudulentos. La cita siempre es gratuita y solo se agenda en canales oficiales.

## Cómo sacar cita

1. Ingresa al portal oficial **citas.sre.gob.mx**.
2. Crea una cuenta o inicia sesión (puedes usar tu cuenta **Llave MX**).
3. Selecciona el trámite (primera vez, renovación, menor, etc.).
4. Elige la entidad, la oficina, y la fecha y hora disponibles.
5. Confirma la cita y guarda el comprobante.
6. Realiza el **pago de derechos** (en banco, en línea o en caja de la oficina, según el caso).

También puedes agendar por teléfono al **55 8932 4827** (gratuito). Si estás en el extranjero, usa la opción **MiConsulado** en el mismo portal.

## Paso a paso

1. **Reúne tus documentos** (acta certificada, identificación vigente, CURP y, para renovación, el pasaporte anterior).
2. **Agenda tu cita** en citas.sre.gob.mx.
3. **Paga los derechos** según la vigencia y conserva el comprobante.
4. **Acude a tu cita** de manera personal y puntual, con los documentos originales.
5. En la oficina: cotejo de documentos, toma de **fotografía**, **firma y huellas**.
6. **Recoge tu pasaporte** el mismo día (en la mayoría de los casos).

## Tiempo de respuesta

En general el pasaporte se entrega **el mismo día de la cita**. El tiempo en oficina suele ser de un par de horas según la afluencia. En consulados puede variar y, en algunos casos, se entrega días después.

## Vigencia

Se expide con vigencia de **3, 6 o 10 años** (este último solo para mayores de edad) y de **1 año** para menores de 3 años. A mayor vigencia, mayor costo, pero menos renovaciones.

## Problemas comunes

- **No hay citas disponibles:** revisa el portal a distintas horas (se liberan espacios), considera otra delegación o usa la atención telefónica.
- **Gestores o sitios falsos** que cobran por la cita: la cita siempre es gratuita y solo se agenda en citas.sre.gob.mx.
- **Acta con errores o registro extemporáneo:** puede requerir documentos complementarios; obtén una copia certificada reciente.
- **Trámite de menor con un solo padre presente:** sin la firma o documentación del otro padre/tutor (o resolución judicial), no procede.
- **Discrepancia de datos** entre acta, CURP e identificación: corrígelos antes de la cita.
