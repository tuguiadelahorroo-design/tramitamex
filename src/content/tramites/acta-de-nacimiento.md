---
titulo: Acta de nacimiento
tituloLargo: 'Acta de nacimiento en línea: cómo descargar tu copia certificada'
descripcion: 'Descarga tu acta de nacimiento certificada en línea en el portal oficial gob.mx/actas. Costo por estado, paso a paso y preguntas frecuentes 2026.'
resumen: 'La copia certificada de tu acta de nacimiento se descarga en línea en el portal oficial, con la misma validez legal que la de ventanilla. Te decimos cómo, cuánto cuesta por estado y cómo evitar sitios fraudulentos.'
categoria: 'Registro Civil'
institucion: 'renapo'
nivel: 'Federal'
costo: 'Desde ~$75 (varía por estado)'
modalidad: 'En línea y presencial'
tiempo: 'Inmediato'
vigencia: 'No caduca (suelen pedirla reciente)'
urlOficial: 'https://www.gob.mx/actas'
urlOficialTexto: 'Portal oficial de Actas — gob.mx'
actualizado: 2026-06-17
destacado: true
orden: 5
faq:
  - pregunta: '¿La copia en línea tiene la misma validez que la de la oficina?'
    respuesta: 'Sí. Tiene plena validez legal ante autoridades municipales, estatales y federales, gracias a su folio, sello electrónico de SEGOB y código QR de verificación.'
  - pregunta: '¿Cuál es el sitio oficial y único para tramitarla?'
    respuesta: 'gob.mx/actas (o gob.mx/ActaNacimiento). Cualquier otro sitio que no termine en gob.mx puede ser fraudulento.'
  - pregunta: '¿Cuánto cuesta?'
    respuesta: 'Varía por estado, aproximadamente entre 75 y 240 pesos en 2026. El monto exacto lo fija cada entidad y se muestra antes de pagar.'
  - pregunta: '¿Necesito CURP obligatoriamente?'
    respuesta: 'Es lo más rápido, pero si no la tienes o está desactualizada puedes buscar por datos: entidad, nombre completo, fecha de nacimiento y nombres de los padres.'
  - pregunta: '¿Necesito cita para hacerlo en línea?'
    respuesta: 'No. El trámite en línea está disponible 24/7 sin cita. La cita solo aplica para la atención presencial en el Registro Civil.'
  - pregunta: '¿El acta caduca?'
    respuesta: 'Legalmente no caduca, pero muchas instituciones piden que tenga menos de 3 a 6 meses de emitida.'
  - pregunta: '¿Puedo imprimirla en casa y que sea válida?'
    respuesta: 'Sí. Se imprime en hoja blanca tamaño carta y conserva su validez mientras el folio y el código QR se verifiquen correctamente.'
  - pregunta: '¿Cómo verifico que un acta digital es auténtica?'
    respuesta: 'Escaneando el código QR o capturando el folio en los sistemas oficiales de verificación, que confirman autenticidad y coincidencia de datos.'
fuentes:
  - titulo: 'Portal oficial — Acta de Nacimiento (gob.mx/actas)'
    url: 'https://www.gob.mx/actas'
  - titulo: 'SEGOB/RENAPO — El único portal oficial es gob.mx/ActaNacimiento'
    url: 'https://www.gob.mx/segob/renapo/articulos/el-unico-portal-oficial-del-gobierno-de-mexico-para-obtener-la-copia-certificada-de-tu-acta-de-nacimiento-es-www-gob-mx-actanacimiento'
  - titulo: 'Plataforma Nacional del Registro Civil — Requisitos'
    url: 'https://www.miregistrocivil.gob.mx/public/actas/commons/RequisitosTramites.xhtml?idTramite=1'
  - titulo: 'Sistema de Verificación de Actos Registrales (CEVAR)'
    url: 'https://cevar.registrocivil.gob.mx/eVAR/ConsultaFolio.jsp'
---

## Qué es

El **acta de nacimiento** es el documento oficial que emite el Registro Civil para acreditar el nacimiento, la identidad y la nacionalidad de una persona en México. La **copia certificada en línea** es la versión digital (PDF) de esa acta asentada en los libros del Registro Civil. Tiene la **misma validez legal** que la copia impresa de ventanilla: incluye folio único de control, sello electrónico de la Secretaría de Gobernación (SEGOB) y código QR para su verificación. Impresa en hoja blanca tamaño carta es válida ante cualquier autoridad municipal, estatal y federal.

## Para qué sirve

- Acreditar identidad, edad y nacionalidad mexicana.
- Tramitar la CURP, el pasaporte, la credencial del INE y la cartilla militar.
- Inscripciones escolares y trámites educativos (certificados, titulación).
- Trámites bancarios, de seguridad social (IMSS, ISSSTE) y laborales.
- Matrimonio, registro de hijos, sucesiones y trámites notariales.
- Trámites migratorios, visas y procesos consulares.

## Requisitos

- **CURP** de la persona titular del acta (la forma más rápida y segura de búsqueda).
- Si no cuentas con CURP o no está actualizada en RENAPO, puedes buscar por datos: **entidad de registro, nombre completo, fecha de nacimiento y nombres de los padres**, tal como aparecen en el registro original.
- Medio de pago: tarjeta de crédito o débito, referencia bancaria o pago en tienda (según el estado).

## Documentos necesarios

Para la copia certificada **en línea no se entregan documentos físicos**; solo se capturan datos. Conviene tener a la mano:

- La **CURP** (impresa o el dato exacto de 18 caracteres).
- Datos del registro: entidad, nombre completo, fecha de nacimiento y nombres de los padres.
- Tarjeta bancaria o datos para generar la referencia de pago.

## Costo

La copia certificada en línea **sí tiene costo** y **varía según la entidad federativa**, porque cada gobierno estatal fija la tarifa. A junio de 2026 el rango aproximado va de **75 a 240 pesos**. Ejemplos de referencia:

- Ciudad de México: aproximadamente 98–105 pesos
- Estado de México: aproximadamente 86 pesos
- Jalisco: aproximadamente 121 pesos
- Nuevo León: aproximadamente 144 pesos

El monto exacto se muestra en el portal una vez que el sistema localiza tu registro. Solo pagas el derecho que define tu estado; el portal nacional no cobra cargos adicionales.

## Dónde se hace

- **En línea (portal nacional oficial):** **gob.mx/actas** (también como gob.mx/ActaNacimiento). Es el **único portal oficial** del Gobierno de México para obtener la copia certificada; opera 24/7.
- **Presencial:** en las oficinas del **Registro Civil** de tu estado o municipio.

> **Cuidado con el fraude:** existen sitios que imitan al portal oficial, cobran de más o roban datos. El único portal oficial termina en **gob.mx**. Verifica siempre la dirección antes de pagar.

## Cómo sacar cita

Para la copia certificada **en línea no se requiere cita**: el trámite es inmediato y está disponible 24/7. Solo se necesita cita cuando acudes de forma **presencial** al Registro Civil; en ese caso, se agenda en el portal del Registro Civil de tu entidad (cada estado tiene su propio sistema).

## Paso a paso

1. Entra al portal oficial **gob.mx/actas**.
2. Selecciona el trámite **Acta de nacimiento**.
3. Ingresa tu **CURP** (búsqueda recomendada). Si no está actualizada, elige búsqueda **por datos**: entidad, nombre completo, fecha de nacimiento y nombres de los padres.
4. Revisa la **vista previa** y confirma que todo coincida con el registro original.
5. Selecciona la **forma de pago** y cubre el costo.
6. **Descarga el PDF** de tu acta certificada (con folio, sello electrónico y código QR).
7. Imprímela en **hoja blanca tamaño carta** cuando la necesites; conserva el PDF para reimprimirla.

## Tiempo de respuesta

La descarga es prácticamente **inmediata** una vez confirmado el pago. Si pagas por referencia bancaria o en tienda, debes esperar a que se acredite (algunas horas) antes de descargar.

## Vigencia

Legalmente el acta **no caduca**: sigue siendo válida mientras el folio permanezca activo y el código QR pueda verificarse. Sin embargo, muchas instituciones (bancos, escuelas, INE, dependencias) **piden un acta reciente**, por lo general con no más de **3 a 6 meses** de antigüedad. Descarga una copia nueva poco antes del trámite que la requiera.

## Problemas comunes

- **La CURP no aparece o está desactualizada:** usa la búsqueda por datos (entidad, nombre, fecha y nombres de los padres).
- **Datos del acta con errores:** debe corregirse mediante una aclaración o rectificación en el Registro Civil de tu estado (trámite aparte).
- **Sitios fraudulentos:** páginas que imitan al portal y cobran de más. Verifica que la URL sea gob.mx.
- **El pago no se acredita:** revisa el estado de la referencia; con tarjeta suele ser inmediato, con referencia o tienda puede tardar horas.
- **No encuentra el registro:** puede que el estado aún no tenga digitalizado ese libro; acude al Registro Civil presencial.
